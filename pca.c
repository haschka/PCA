...
void print_eigenvectors(FILE* f, data_shape s,
			eigen_space ei, size_t n_vectors) {

  size_t i, j;
  
  size_t n_features = s.n_features;
  
  for(i = 0; i < n_vectors; i++) {
    fprintf(f, "eigenvector_%lu\t",i); 
    for(j = 0; j < n_features-1; j++) {
      fprintf(f,"%.6E\t",ei.eigenvectors[(n_features-1-i)*n_features+j]);
    }
    j++;
    fprintf(f,"%.6E\n",ei.eigenvectors[(n_features-1-i)*n_features+j]);
  }
}
...
double* feature_covariance_matrix(double* data, double* means, data_shape s,
  ...
  if(c_matrix == NULL) {
    printf("Not enough memory to hold a correlation matrix of size %li\n",
	   s.n_features*s.n_features);
    _exit(1);
  }
...
int main(int argc, char** argv){
  ...
  sscanf(argv[5],"%lu",&dimensions);
  sscanf(argv[6],"%lu",&n_threads);
  ...
  print_eigenvectors(vec_file, shape, ei, dimensions);
